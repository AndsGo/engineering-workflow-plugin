...
- **T1 Standard** — bounded bugfix/feature, including **wide-mechanical** work (one pattern repeated across many files, no per-site design choice; announce the expected breadth): spec-lite + one test + one `structured-review`.
- **T2 Substantial** — real design choices / intent must be excavated / oracle must be designed / interacting-subsystem breadth: full flow (brainstorming → writing-plans → plan-review-personas → subagent-driven-development → structured-review). Size alone is not a tier signal.

An **invariant floor** holds at every tier (define "correct" first; a check that can actually fail; evidence before "done"; never auto-execute irreversible/outward-facing actions; learnings discipline), plus a **completion-time checkpoint** that re-scans the actual diff before "done": a change that turns out to touch a security path (→ mandatory `security-audit`), outgrow its announced scope (or exceed ~5 files when no breadth was announced → plan-review), or perform a surprise irreversible op (→ human approval) is caught and escalated even if it started trivial — and the checkpoint runs *before* any push/deploy. T1+ work announces its tier so you can veto a misclassification in one glance.
